...
// ===============================
// CONFIG: URL de verificación (QR)
// ===============================
// Esto apunta a verificar.html en la misma carpeta/sitio:
const URL_VERIFICACION_BASE = `${location.origin}${location.pathname.replace(/\/[^/]*$/, "")}/verificar.html?cert=`;
// =============================== 
...
async function generarPDFMedievalConQR() {
  ...
  // URL QR
  const urlVerificacion = `${URL_VERIFICACION_BASE}${encodeURIComponent(certId)}`;
  ...
  if (qrDataURL) {
    ...
    // Referencia directa a tu sello
    const selloLineY = 252; // doc.line(W - 94, 252, W - 24, 252)

    // Antes: 108 (8 + 100). Ahora bajamos 5 cm (=50 mm): 108 - 50 = 58
    const gap = 58;

    // Centrado real en la hoja
    const x = (W / 2) - (size / 2);

    let y = selloLineY - gap - size; // arriba del sello con gap fijo

    // Seguridad: evita que el QR se meta demasiado arriba en el marco
    const marcoMinY = 20;
    ...
    // textura sutil sobre el marco (no afecta el QR)
...
// QR a Base64
// ===============================
function generarQRBase64(texto, sizePx) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      ...
      const out = (img && img.src) ? img.src : (canvas ? canvas.toDataURL("image/png") : null);
...
// ===============================
// Verificación demo (localStorage)
// ===============================
function registrarCertificadoLocal(certId, nombre) {
  try {
    const key = "VI_CERTS";
    const arr = JSON.parse(localStorage.getItem(key) || "[]");
    arr.push({ certId, nombre, fechaISO: new Date().toISOString() });
...
// ===============================
// Utilidades de dibujo PDF
// ===============================
function aplicarOpacidad(doc, opacity, fn) {
  try {
    if (doc.GState) {
      const g = new doc.GState({ opacity, "stroke-opacity": opacity });
...
function textoEnBloqueCentrado(doc, text, centerX, startY, maxWidth, lineHeight) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (doc.getTextWidth(test) > maxWidth && i > 0) {
      doc.text(line.trim(), centerX, y, { align: "center" });
      line = words[i] + " ";
      y += lineHeight;
    } else {
      line = test;
  ...
  if (line.trim()) doc.text(line.trim(), centerX, y, { align: "center" });
}

// Ornamento esquina (simple pero elegante)
function adornarEsquina(doc, x, y, corner) {
  // corner: 1 TL, 2 TR, 3 BL, 4 BR
  const s = 14;
  const dx = (corner === 2 || corner === 4) ? -1 : 1;
  const dy = (corner === 3 || corner === 4) ? -1 : 1;

  doc.line(x, y, x + dx * s, y);
  doc.line(x, y, x, y + dy * s);
  doc.line(x + dx * 6, y, x + dx * 10, y + dy * 4);
  doc.line(x, y + dy * 6, x + dx * 4, y + dy * 10);
  doc.circle(x + dx * 4.2, y + dy * 4.2, 1.2, "S");
  doc.circle(x + dx * 8.2, y + dy * 8.2, 1.0, "S");
}

// Ojo de Horus vector (más limpio, no “falso” por ser procedural)
function dibujarOjoHorusVector(doc, cx, cy, size) {
  const oro = [170, 130, 55];
  const oroOsc = [120, 85, 35];
  const w = size * 1.15;
  const h = size * 0.55;

  doc.setDrawColor(...oroOsc);
  doc.setLineWidth(1.2);

  // párpado superior/inferior
  poly(doc, [
    [cx - w, cy],
    [cx - w * 0.55, cy - h * 0.95],
    [cx, cy - h * 1.05],
  ...
  doc.circle(cx, cy, size * 0.18, "S");
  doc.setFillColor(45, 30, 15);
  doc.circle(cx, cy, size * 0.08, "F");

  doc.setDrawColor(...oro);
  doc.setLineWidth(0.7);
  doc.circle(cx, cy, size * 0.13, "S");

  // línea lágrima y cola
  doc.setDrawColor(...oroOsc);
